function windanim(model,logs,config)
...
% Modified by: Yerai Peña-Sanchez

cfg.movie.go                = questdlg('Do you want to record the video of the animation?');
...
if cfg.movie.go
    video                   = VideoWriter('WindFieldAnimation.avi');
    open(video);
end
...
for step = 1:numframes-1
    ...
    if(cfg.movie.go)
        if step == 1
            f = msgbox('Please, change the things you would like to change on the figure to record it (limits, aspect ratio, etc.) and close this dialog to start the recording.');
            waitfor(f)
        end
        frame               = getframe(gcf);
