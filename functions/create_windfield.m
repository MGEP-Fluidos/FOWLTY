function [wind,file,path] = create_windfield(windData)
...
if nargin == 1
    if(isfield(windData,'Ua'))  % If just information from old wind field is provided
        U0      = windData.Ua;
        Ti      = windData.Ti;
        d       = windData.d;
        Lx      = windData.Lx;
        ...
        while(true)
            if(~exist('U0') | error)
                qs          = {'Mean wind speed [m/s]', ...
                               ...
                               'Simulation time [s]',...
                               'Sampling time [s]'};
                answer      = {'12','.1',num2str(Lx),num2str(Ly),'15','1000','0.5'};
            ...
            if(~error)
                ...
                defdesc{1}  = ['Mean:' num2str(U0) ' m/s Turbulence: ' num2str(Ti) ' SimTime:' num2str(SimTime)  ' Ts:' num2str(Ts) ];
